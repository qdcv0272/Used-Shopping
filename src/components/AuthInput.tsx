...
export const AuthInputWithCheck = forwardRef<
  HTMLInputElement,
  AuthInputWithCheckProps
>(
  (
    {
      label,
      ...
      checkMessage,
      onCheck,
      checkButtonLabel = "중복확인",
      checkedButtonLabel = "완료",
      className = "",
      ...props
    },
    ref,
    ...
    return (
      <div className="field role-field">
        ...
        {error && <small className="error-text">{error}</small>}
        {!error && checkMessage && (
          <small className="success-text">{checkMessage}</small>
        )}
      </div>
    );
  },
